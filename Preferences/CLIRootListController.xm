#include "CLIRootListController.h"
#import "spawn.h"

@implementation CLIRootListController
...
- (instancetype)init
{
    ...
    if (self)
	{
        CLIAppearanceSettings *appearanceSettings = [[CLIAppearanceSettings alloc] init];
        ...
        self.titleLabel.text = @"CustomAutoLockInterval";
...
{
    if (scrollView.contentOffset.y > [CLIRootHeaderView headerH] / 2.0) [UIView animateWithDuration: 0.2 animations: ^{ self.titleLabel.alpha = 1.0; }];
...
- (void)reset: (PSSpecifier*)specifier
{
    UIAlertController *reset = [UIAlertController
        alertControllerWithTitle: @"CustomAutoLockInterval"
	...
	UIAlertAction *confirmAction = [UIAlertAction actionWithTitle: @"Confirm" style: UIAlertActionStyleDestructive handler:
        ...
        {
            [[[HBPreferences alloc] initWithIdentifier: @"com.johnzaro.customautolockintervalprefs"] removeAllObjects];

            NSFileManager *manager = [NSFileManager defaultManager];
            [manager removeItemAtPath:@"/var/mobile/Library/Preferences/com.johnzaro.customautolockintervalprefs.plist" error: nil];
